...
public abstract class Utility {
	...
	while (i < size) {
		...
		stream.readUnsignedByte();
		instruction = new INVOKEDYNAMIC(index);
		break;
	    case INVOKEINTERFACE:
		index = stream.readUnsignedShort();
		nargs = stream.readUnsignedByte();
		stream.readUnsignedByte();
		instruction = new INVOKEINTERFACE(index, nargs);
		break;
	    case INVOKESPECIAL:
		index = stream.readUnsignedShort();
		instruction = new INVOKESPECIAL(index);
		break;
	    case INVOKESTATIC:
		index = stream.readUnsignedShort();
		instruction = new INVOKESTATIC(index);
		break;
	    case INVOKEVIRTUAL:
		index = stream.readUnsignedShort();
    ...
    public static String mehtodSignatureToString(String signature) {
	return null;
    }
